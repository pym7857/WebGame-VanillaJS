...
스크린.addEventListener('click', function() {
	if (스크린.classList.contains('waiting')) { // 대기 상태 (파란색)
		스크린.classList.remove('waiting');
		스크린.classList.add('ready');
		스크린.textContent = '준비하시고, 초록색이 되면 클릭하세요!'
		타임아웃 = setTimeout(function() {
			시작시간 = new Date();
			스크린.click();
		}, Math.floor(Math.random() * 1000) + 2000); // 2000 ~ 3000 사이 수 
	} else if (스크린.classList.contains('ready')) { // 준비 상태 (빨간색)
		if (!시작시간) { // 부정 클릭 
			clearTimeout(타임아웃);
	...
	} else if (스크린.classList.contains('now')) { // 시작 상태 (형광색)
		끝시간 = new Date();
